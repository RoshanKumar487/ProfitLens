        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-8 p-0 print:hidden sticky left-0 bg-background"></TableHead>
                  <TableHead className="w-[250px] sticky left-[2rem] bg-background">Employee</TableHead>
                ...
                {isLoading ? (
                  ...
                  filteredAndCalculatedData.map(emp => (
                    <TableRow key={emp.id} className="group">
                      <TableCell className="p-0 print:hidden sticky left-0 bg-background group-hover:bg-muted/50"><div className="flex items-center justify-center"><TooltipProvider><Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 rounded-full opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleInsertRow(emp.id)}><PlusCircle className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent side="right"><p>Insert row below</p></TooltipContent></Tooltip></TooltipProvider></div></TableCell>
                      <TableCell className="sticky left-[2rem] bg-background group-hover:bg-muted/50">{emp.isNew ? (<Input placeholder="Enter Employee Name" value={emp.name} onChange={e => handleInputChange(emp.id, 'name', e.target.value)} />) : (<div className="flex items-center gap-2"><Avatar><AvatarImage src={emp.profilePictureUrl} /><AvatarFallback>{getInitials(emp.name)}</AvatarFallback></Avatar><span className="font-medium">{emp.name}</span></div>)}</TableCell>
              ...
              {filteredAndCalculatedData.length > 0 && (
                <TableFooter>
                    <TableRow className="font-bold bg-muted/50">
                        <TableCell className="print:hidden sticky left-0 bg-muted/50"></TableCell>
                        <TableCell className="sticky left-[2rem] bg-muted/50">Totals</TableCell>
